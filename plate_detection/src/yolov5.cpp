...
void imageCallback(const sensor_msgs::Image::ConstPtr& raw_image) {
    ...
    if (res.size() != 0) {
        ...
        for (size_t j = 0; j < res.size(); j++) {
            if ((res[tmp].bbox[2] * res[tmp].bbox[3]) < (res[j].bbox[2] * res[j].bbox[3])) {
    	        tmp = j;
	    }
    	}
	...
	res[tmp].bbox[2] += 10;
	res[tmp].bbox[3] += 10;
        cv::Rect r = get_rect(img, res[tmp].bbox);
	...
	//cv::imwrite("/root/catkin_ws/src/plate_detection/img/" + std::to_string(fcount) + ".jpg", frame(r));
    }
}
...
int main(int argc, char** argv) {
    ...
    pub_image = nh.advertise<sensor_msgs::Image>("/ocr_sub", 5);
